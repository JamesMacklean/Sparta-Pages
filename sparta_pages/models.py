...
class SpartaCourseCodes(models.Model):
    ...
    created_at = models.DateTimeField(auto_now_add=True)
    courseCode = models.CharField(max_length=2, choices=COURSE_CHOICES, default=SP101)

    class Meta:
        verbose_name_plural = "10. Sparta Course Codes"

    def __str__(self):
        return "{}: {}".format(self.profile.user.username, self.course.name)

    def sp101(self):
        if self.courseCode != self.SP101:
            self.courseCode = self.SP101
            self.save()

    def sp201(self):
        if self.courseCode != self.SP201:
            self.courseCode = self.SP201
            self.save()

    def sp301(self):
        if self.courseCode != self.SP301:
            self.courseCode = self.SP301
            self.save()

    def sp401(self):
        if self.courseCode != self.SP401:
            self.courseCode = self.SP401
            self.save()
